...
// Одна строка фактического продуктового расхода из базы «Денег» (HomeAccounting).
public sealed record MoneyExpense(
    string Date, string Category, string Subcategory,
    string Account, decimal Amount, string Note);

/// <summary>
/// Чтение ФАКТИЧЕСКИХ расходов на продукты из базы HomeAccounting («Деньги»).
/// ТОЛЬКО ЧТЕНИЕ. По правилу офиса расходы ведутся исключительно в «Деньгах»;
/// «Еда» их лишь показывает. База: %LocalAppData%\HomeAccounting\homeaccounting.db.
/// </summary>
public static class HomeAccountingReader
{
    ...
    // Категория «Денег» с расходами на продукты.
    private static readonly string[] FoodCategories = { "Продукты питания" };

    public static bool IsAvailable => File.Exists(DbPath);

    public static IReadOnlyList<MoneyExpense> GetFoodExpenses(int? year = null)
    {
        var result = new List<MoneyExpense>();
        if (!IsAvailable) return result;
        try
        {
            // Mode=ReadOnly — гарантия, что «Еда» ничего не изменит в чужой базе.
            using var conn = new SqliteConnection($"Data Source={DbPath};Mode=ReadOnly;Cache=Shared");
            conn.Open();

            var inCats = string.Join(",", FoodCategories.Select((_, i) => "$c" + i));
            ...
            for (int i = 0; i < FoodCategories.Length; i++)
                cmd.Parameters.AddWithValue("$c" + i, FoodCategories[i]);
